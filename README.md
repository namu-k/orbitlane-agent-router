...
Routing can only fill in a model the Agent tool accepts. A lane bound to a token outside that set — a pinned full model identifier, for instance — is still projected as guidance, but the guard leaves such spawns alone rather than rewriting them into a call the runtime would reject. The generated report marks each route with `injectable` so this is visible at install time. OrbitLane deliberately does not map a pinned identifier onto an alias, because the alias would resolve to whichever model it currently points at.
